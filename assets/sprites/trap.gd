...
func _ready() -> void:
	...
	# 2. Registrar no grupo que o GameManager chama
	add_to_group("trap")
	
	# 3. Configurações específicas
	faction = Faction.TRAP
	is_immobile = true
	is_hittable = false # Armadilhas de chão não podem ser atacadas
	...
	if _state == State.ACTIVE:
		...
		for target in targets:
			if target != self and target.is_hittable:
				# Causa dano em Player ou Inimigos
...
func _update_visuals() -> void:
	...
	# Num jogo final, você usaria AnimationPlayer aqui.
	if danger_indicator:
